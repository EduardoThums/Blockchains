...
public class Util {
	...
	public FabricUser writeFabricUser(final FabricUser fabricUser) throws Exception {
		final String directoryPath = Config.USERS_DIRECTORY_PATH.getValue() + fabricUser.getAffiliation();
	...
	public FabricUser readFabricUser(final String affiliation, final String username) throws Exception {
		final String filePath = Config.USERS_DIRECTORY_PATH.getValue() + affiliation + "/" + username + ".ser";
	...
	public void cleanUp() {
		final File directory = new File(Config.USERS_DIRECTORY_PATH.getValue());
