...
public class KafkaServiceImpl implements KafkaService {
	...
	public KafkaServiceImpl(IPFSService ipfsService, FabricService fabricService) {
	...
	public void startConsumer() throws Exception {
		while (true) {
			...
			for (final ConsumerRecord<Long, byte[]> record : consumerRecords) {
				...
				hashList.forEach(hash -> log.info("IPFS hash {}", hash));

				fabricService.createTransaction(hashList);
//						.forEach(transactionHash ->
////								log.info("Transaction completed with {} hash", transactionHash)
//						);
			}
